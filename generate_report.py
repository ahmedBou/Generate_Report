...
# Department at your company wants you to find out how many people are in each department. 
# You need to write a Python script that reads a CSV file containing a list of the employees 
# in the organization, counts how many people are in each department, and then generates a report
# using this information.The output of this script will be a plain text file.

# if you work on cloud The script must begins with a line containing the #! character combination, 
# which is commonly called hash bang or shebang, and continues with the path to the interpreter.
# If the kernel finds that the first two bytes are #! then it uses the rest of the line as 
# an interpreter and passes the file as an argument. We will use the following shebang in this script:

#!/usr/bin/env python3

# 1.Convert employee data to dictionary

# The goal of the script is to read the CSV file and generate a report with the total number of 
# people in each department. To achieve this, we will divide the script into three functions.

# Let's start with the first function: read_employees(). This function receives a CSV file as a 
# parameter and returns a list of dictionaries from that file. For this, we will use the CSV module.
...
# Rather than passing each of these parameters to the reader and writer separately, we group them together conveniently into a dialect
# object.
...
# Pass the path to employees.csv as a parameter to the function. Print the variable employee_list 
# to check whether it returns a list of dictionaries.
...
# We will return this dicationary in the format department:amount,
...
# Now, call this function by passing the employee_list from the previous section. 
# Then, save the output in a variable called dictionary. Print the variable dictionary.
dictionary = process_data(employee_list)
print(dictionary)

# 3.Generate a report
# Next, we will write the function write_report. This function writes a dictionary of department: 
# amount to a file.
...
#  This function requires a dictionary, from the previous section, and report_file, 
# an output file to generate report, to both be passed as parameters.
...
# Once you open the file for writing, iterate through the dictionary and use write() on the file to store the data.
  ...
  with open(report_file, "w+") as f:
    ...
    f.close()
# Now call the function write_report() by passing a dictionary variable from the previous section and 
# also passing a report_file. The report_file passed within this function should be similar
#  to /home/<username>/data/report.txt. 
